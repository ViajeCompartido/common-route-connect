import { Sparkles, Clock, Users, Send } from 'lucide-react';
...
export const CompatiblePassengersBlock = (props: DriverProps) => {
  const [offerTarget, setOfferTarget] = useState<{ passengerId: string; rideRequestId: string; seats: number; name: string } | null>(null);
          ...
          <div key={m.id} className="flex items-center gap-2 bg-card rounded-lg p-2 border border-border">
            ...
            {m.hasOffer ? (
              ...
              <Button
                ...
                onClick={() => setOfferTarget({ passengerId: m.passenger_id, rideRequestId: m.id, seats: m.seats, name: m.passenger_name })}
              >
...
export const CompatibleTripsBlock = (props: PassengerProps) => {
  const [offerTarget, setOfferTarget] = useState<{ tripId: string; name: string } | null>(null);
          ...
          <div key={m.id} className="flex items-center gap-2 bg-card rounded-lg p-2 border border-border">
            ...
            {m.hasOffer ? (
              ...
              <Button
                ...
                onClick={() => setOfferTarget({ tripId: m.id, name: m.driver_name })}
              >
