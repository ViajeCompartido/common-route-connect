...
const SearchForm = ({ onSearch, compact = false }: SearchFormProps) => {
  ...
  const activeFilterCount = [
    filters.acceptsPets,
    filters.driverHasPet,
    filters.allowsLuggage,
    filters.minRating > 0,
    filters.minSeats > 1,
          ...
          >
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label htmlFor="pets-search" className="text-sm flex items-center gap-2">
                  <PawPrint className="h-4 w-4 text-accent" /> Acepta mascotas
                </Label>
                <Switch
                  id="pets-search"
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  id="driver-pet"
                  checked={filters.driverHasPet}
                  onCheckedChange={(v) => setFilters({ ...filters, driverHasPet: v })}
                />
              </div>
              <div className="flex items-center justify-between">
                <Label htmlFor="luggage-search" className="text-sm flex items-center gap-2">
                  <Luggage className="h-4 w-4" /> Permite equipaje grande
                </Label>
                <Switch
                  id="luggage-search"
            ...
            <div className="space-y-2">
              <Label className="text-sm flex items-center gap-2">
                ...
                Reputación mínima: {filters.minRating > 0 ? `${filters.minRating}+ ★` : 'Cualquiera'}
              </Label>
              <Slider
            ...
            <div className="space-y-2">
              <Label className="text-sm flex items-center gap-2">
                ...
                Asientos mínimos: {filters.minSeats}
              </Label>
              <Slider
                value={[filters.minSeats]}
                onValueChange={([v]) => setFilters({ ...filters, minSeats: v })}
                min={1}
                max={6}
                step={1}
                className="py-2"
            ...
            {activeFilterCount > 0 && (
              <button
                type="button"
                onClick={() => setFilters({ ...filters, acceptsPets: false, driverHasPet: false, allowsLuggage: false, minRating: 0, minSeats: 1 })}
