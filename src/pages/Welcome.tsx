...
const Welcome = () => {
  ...
  return (
    <div className="min-h-screen gradient-ocean flex flex-col items-center justify-between px-6 py-10">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center max-w-sm">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-10">
          {[
            ...
            <motion.div key={feat.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + i * 0.1 }} className="flex items-center gap-1.5 bg-primary-foreground/15 backdrop-blur-sm rounded-full px-3.5 py-2 border border-primary-foreground/10">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="w-full max-w-sm space-y-3">
        <Button onClick={() => navigate('/register')} className="w-full h-14 gradient-accent text-primary-foreground rounded-2xl text-base font-semibold shadow-ocean">Crear cuenta</Button>
        <Button onClick={() => navigate('/login')} variant="outline" className="w-full h-14 rounded-2xl text-base font-semibold bg-primary-foreground/15 border-primary-foreground/30 text-primary-foreground hover:bg-primary-foreground/25">Ya tengo cuenta</Button>
        <p className="text-center text-primary-foreground/60 text-[11px] mt-4">
          Al crear una cuenta aceptás los{' '}
          <button onClick={() => navigate('/terms')} className="underline hover:text-primary-foreground/80">términos y condiciones</button>
          {' '}y la{' '}
          <button onClick={() => navigate('/privacy')} className="underline hover:text-primary-foreground/80">política de privacidad</button>
        </p>
      </motion.div>
    </div>
  );
